...
class Card(BaseModel):
    ...
    id: str = Field(regex=r'^[a-z_][a-z0-9_]*$', description="Unique card identifier")
...
class Enemy(BaseModel):
    ...
    id: str = Field(regex=r'^[a-z_][a-z0-9_]*$', description="Unique enemy identifier")
    name: str = Field(min_length=1, max_length=50, description="Display name")
...
class Event(BaseModel):
    ...
    id: str = Field(regex=r'^[a-z_][a-z0-9_]*$', description="Unique event identifier")
    name: str = Field(min_length=1, max_length=50, description="Display name")
...
class Deck(BaseModel):
    ...
    id: str = Field(regex=r'^[a-z_][a-z0-9_]*$', description="Unique deck identifier")
    name: str = Field(min_length=1, max_length=50, description="Display name")
...
class CardsFile(BaseModel):
    ...
    @validator('__root__')
    def validate_card_ids_match_keys(cls, v):
        """Ensure card IDs match their dictionary keys."""
        for key, card in v.items():
            if card.id != key:
                raise ValueError(f"Card ID '{card.id}' doesn't match key '{key}'")
        return v


class EnemiesFile(BaseModel):
    """Schema for enemies YAML files."""
    __root__: Dict[str, Enemy]
    
    @validator('__root__')
    def validate_enemy_ids_match_keys(cls, v):
        """Ensure enemy IDs match their dictionary keys."""
        for key, enemy in v.items():
            if enemy.id != key:
...
class EventsFile(BaseModel):
    ...
    __root__: Dict[str, Event]
    
    @validator('__root__')
    def validate_event_ids_match_keys(cls, v):
        """Ensure event IDs match their dictionary keys."""
        for key, event in v.items():
            if event.id != key:
                raise ValueError(f"Event ID '{event.id}' doesn't match key '{key}'")
        return v


class DecksFile(BaseModel):
    """Schema for decks YAML files."""
    __root__: Dict[str, Deck]
    
    @validator('__root__')
    def validate_deck_ids_match_keys(cls, v):
        """Ensure deck IDs match their dictionary keys."""
        for key, deck in v.items():
            if deck.id != key:
